...
export class AntiClockWiseSubMachine extends SubStateMachine {
  ...
  constructor(public fms: StateMachine) {
    super(fms);
    this.stateMachines.set(PARAMS_NAME_DIRECTION_ENUM.TOP, new State(fms, `${PATH}/left`));
    this.stateMachines.set(PARAMS_NAME_DIRECTION_ENUM.RIGHT, new State(fms, `${PATH}/top`));
    this.stateMachines.set(PARAMS_NAME_DIRECTION_ENUM.BOTTOM, new State(fms, `${PATH}/right`,));
    this.stateMachines.set(PARAMS_NAME_DIRECTION_ENUM.LEFT, new State(fms, `${PATH}/bottom`));
  }
